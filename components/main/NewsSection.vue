...
const api = getApi()
const { data: newsData, pending } = await useAsyncData('news', () => api.get(endpoints.news).then(r => r.data))

const allItems = computed<NewsItem[]>(() => {
  if (!newsData.value) return []
  return Array.isArray(newsData.value) ? newsData.value : []
})

// Get display date: published_at if available, otherwise created_at
function getDisplayDate(item: NewsItem): string | null {
  ...
  <section class="pb-[160px] max-md:pb-[60px] container">
    ...
    <template v-else>
      ...
      <div class="mb-[28px] flex items-center justify-between gap-4">
        ...
        <!-- Desktop Pager (← — →) -->
        <div v-if="allItems.length > 0" class="hidden md:flex items-center gap-5 rounded-xl border border-m2-gray-light px-3 py-2 text-m2-dark/70">
      ...
      <div v-if="allItems.length > 0" class="hidden md:grid gap-[24px] md:grid-cols-3">
        <article
          v-for="n in items"
          :key="n.id"
        ...
        >
          <p v-if="getDisplayDate(n)" class="text-[12px] leading-[16px] text-m2-gray mb-[40px]">
            {{ formatDate(getDisplayDate(n)) }}
          </p>
          ...
          <div class="text-[14px] leading-[22px] text-m2-gray" v-html="n.content || n.excerpt"></div>
        </article>
      </div>

      <!-- Mobile Single Card View -->
      <div v-if="allItems.length > 0" class="md:hidden mb-[40px]">
        <article
          v-if="currentNews"
          :key="currentNews.id"
          class="rounded-2xl border border-m2-gray-light bg-white p-[21px] min-h-[350px] shadow-[0_1px_0_rgba(17,24,39,0.03)]"
        >
          <p v-if="currentNews.published_at" class="text-[12px] leading-[16px] text-m2-gray mb-[26px]">
          ...
          <div class="text-[14px] leading-[22px] text-m2-gray" v-html="currentNews.content || currentNews.excerpt"></div>
        </article>
        ...
        <div class="flex items-center justify-between mt-6 px-4 h-[50px] py-[12px] border border-m2-gray-light rounded-[10px] bg-white">
          ...
          <span class="text-m2-dark text-[14px] font-bold">{{ currentIndex + 1 }} из {{ allItems.length }}</span>
          <button
            @click="nextMobile"
      ...
      <!-- Empty state -->
      <div v-if="!pending && allItems.length === 0" class="text-center py-[60px]">
        <p class="text-m2-gray">Новостей пока нет</p>
      </div>
    </template>
  </section>
</template>